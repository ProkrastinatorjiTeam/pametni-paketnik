...
function BoxDetailModal({box, isOpen, onClose, onBoxUpdated}) {
    ...
    const fetchPrintHistory = async (boxId) => {
        ...
        try {
            const response = await axios.get(`/order/by-box/${boxId}`);
            setPrintHistory(response.data || []);
        } catch (err) {
            console.error('Napaka pri pridobivanju zgodovine naročil:', err);
        } finally {
    ...
    const fetchUnlockHistory = async (boxId) => {
        ...
        try {
            const response = await axios.get(`/box/history/${boxId}`);
    ...
    const handleSubmit = async (e) => {
        ...
        setSuccessMessage('');
        try {
            const response = await axios.patch(`/box/update/${box._id}`, editData);
    ...
    const handleDeleteBox = async () => {
        ...
        try {
            await axios.delete(`/box/remove/${box._id}`);
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            {order.status}
                            ...
                            )) : <li>Ni zgodovine naročil za ta box.</li>}
                        </ul>
                );
            case 'unlockHistory':
                return (
                        ...
                        <ul className="data-list">
                            {unlockHistory.length > 0 ? unlockHistory.map(event => (
                                <li key={event._id} className="data-list-item history-item-unlock">
                                    ...
                                    <span
                                        className={event.authorized ? 'status-text-success' : 'status-text-error'}>{event.authorized ? 'Pooblaščen' : 'Nepooblaščen'}</span>
                ...
                return (
                    <form id="edit-box-form" onSubmit={handleSubmit} className="edit-box-form">
                        <div className="form-group">
                            ...
                            <input type="text" id="editBoxName" name="name" value={editData.name}
                                   onChange={handleChange} required/>
                        </div>
                        <div className="form-group">
                            <label htmlFor="editBoxLocation">Lokacija:</label>
                            <input type="text" id="editBoxLocation" name="location" value={editData.location}
                                   onChange={handleChange}/>
                        </div>
                        <div className="form-group">
                            <label htmlFor="editBoxPhysicalId">Fizični ID:</label>
                            <input type="text" id="editBoxPhysicalId" name="physicalId" value={editData.physicalId}
                                   onChange={handleChange} required/>
                        </div>
                    </form>
                );
            ...
            <div className="modal-content admin-modal-content large-modal-content">
                ...
                <div className="tab-navigation">
                    <button onClick={() => setActiveTab('details')}
                            className={activeTab === 'details' ? 'active' : ''}>Podrobnosti
                    </button>
                    <button onClick={() => setActiveTab('printHistory')}
                            className={activeTab === 'printHistory' ? 'active' : ''}>Zgodovina naročil
                    </button>
                    <button onClick={() => setActiveTab('unlockHistory')}
                            className={activeTab === 'unlockHistory' ? 'active' : ''}>Zgodovina odpiranj
                    </button>
                </div>
                ...
                <div className="modal-footer">
                    ...
                    <div className="footer-button-group">
                        {activeTab === 'details' && (
                            <>
                                <button type="button" onClick={handleDeleteBox} className="action-button-danger"
                                        disabled={isLoading}>
                                    Izbriši Box
                                </button>
                                <button type="submit" form="edit-box-form" className="action-button-primary"
                                        disabled={isLoading}>
                                    Shrani spremembe
                                </button>
                            </>
