                        ...
                        <ul className="data-list">
                            {orders.length > 0 ? orders.map(order => (
                                <li key={order._id} className="data-list-item history-item-order">
                                    ...
                                    <span>{order.product?.price != null ? `€${order.product.price.toFixed(2)}` : 'N/A'}</span>
                                    <span
                                        className={`status-badge status-${order.status?.toLowerCase().replace(/\s+/g, '-')}`}>{order.status}</span>
            ...
            <div className="modal-content admin-modal-content large-modal-content">
                ...
                <div className="tab-navigation">
                    ...
                    <button onClick={() => setActiveTab('orders')}
                            className={activeTab === 'orders' ? 'active' : ''}>Zgodovina naročil
                    </button>
                    <button onClick={() => setActiveTab('unlocks')}
                            className={activeTab === 'unlocks' ? 'active' : ''}>Zgodovina odpiranj
                    </button>
                </div>
